...
public class OtpAuthenticatorConfiguration : IEntityTypeConfiguration<OtpAuthenticator>
{
    public void Configure(EntityTypeBuilder<OtpAuthenticator> builder)
    {
        ...
        builder.Property(oa => oa.UpdatedDate).HasColumnName("UpdatedDate").IsRequired(false);
        builder.Property(oa => oa.DeletedDate).HasColumnName("DeletedDate").IsRequired(false);

        builder.HasIndex(oa => oa.UserId, "UK_OtpAuthenticators_UserId").IsUnique();
