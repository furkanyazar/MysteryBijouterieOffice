...
public class AuthManager : IAuthService
{
    ...
    private readonly IUserGroupOperationClaimRepository _userGroupOperationClaimRepository;

    public AuthManager(
        IUserGroupOperationClaimRepository userGroupOperationClaimRepository,
        IRefreshTokenRepository refreshTokenRepository,
        ITokenHelper tokenHelper,
        IConfiguration configuration
    )
    {
        _userGroupOperationClaimRepository = userGroupOperationClaimRepository;
    ...
    {
        IList<OperationClaim> operationClaims = await _userGroupOperationClaimRepository
            .Query()
            .AsNoTracking()
            .Where(p => p.UserGroupId == user.UserGroupId)
            .Select(p => new OperationClaim { Value = p.OperationClaim.Value })
            .ToListAsync();
        operationClaims.Add(new() { Value = GeneralOperationClaims.User });
